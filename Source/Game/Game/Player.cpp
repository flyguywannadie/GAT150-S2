...
bool Player::InView(max::vec2& point)
{
	...
	inview = ((max::RadToDeg(transform.rotation) + m_viewAngle) > max::RadToDeg(atan2(point.y, point.x)) && (max::RadToDeg(transform.rotation) - m_viewAngle) > max::RadToDeg(atan2(point.y, point.x)));
