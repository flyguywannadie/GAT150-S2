...
namespace max
{
    ...
    bool Json::Read(const rapidjson::Value& value, const std::string& name, max::vec2& data, bool required)
    {
    ...
    bool Json::Read(const rapidjson::Value& value, const std::string& name, max::Color& data, bool required)
    {
        // check if 'name' member exists and is an array with 2 elements
    ...
    bool Json::Read(const rapidjson::Value& value, const std::string& name, max::Rect& data, bool required)
    {
        // check if 'name' member exists and is an array with 2 elements
        if (!value.HasMember(name.c_str()) || !value[name.c_str()].IsArray() || value[name.c_str()].Size() != 4)
        {
